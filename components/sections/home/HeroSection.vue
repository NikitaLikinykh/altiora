<template>
  <section
    class="relative isolate -z-10 overflow-hidden bg-gradient-to-b from-indigo-100/20 pt-14"
  >
    <!-- Decorative Background -->
    <div
      aria-hidden="true"
      class="absolute inset-y-0 right-1/2 -z-10 -mr-96 w-[200%] origin-top-right skew-x-[-30deg] bg-white shadow-xl shadow-indigo-600/10 ring-1 ring-indigo-50 sm:-mr-80 lg:-mr-96"
    />
    <div class="mx-auto max-w-7xl px-6 py-32 sm:py-40 lg:px-8">
      <div
        class="mx-auto max-w-2xl lg:mx-0 lg:grid lg:max-w-none lg:grid-cols-2 lg:gap-x-16 lg:gap-y-8 xl:grid-cols-1 xl:grid-rows-1 xl:gap-x-8"
      >
        <!-- Title -->
        <h1
          class="max-w-2xl text-balance text-5xl font-semibold tracking-tight text-gray-900 sm:text-6xl lg:col-span-2 xl:col-auto"
        >
          Altiora — онлайн-школа для українських дітей у Польщі
        </h1>
        <!-- Description -->
        <div class="mt-6 max-w-xl lg:mt-0 xl:col-end-1 xl:row-start-1">
          <p class="text-pretty text-lg font-medium text-gray-500 sm:text-xl/8">
            Наша місія — допомогти українським дітям адаптуватися до польської
            системи освіти. Обирайте потрібний предмет і клас для індивідуальних
            занять із досвідченими викладачами. Уроки проводяться за польськими
            підручниками, що допомагає краще зрозуміти контекст місцевої
            шкільної програми. Ми створюємо комфортні умови для навчання, щоб
            ваші діти почувалися впевнено і досягали успіхів у освіті.
          </p>
        </div>
        <!-- Image -->
        <NuxtImg
          alt=""
          src="/images/home/online.jpg"
          width="1200"
          height="800"
          class="mt-10 aspect-[6/5] w-full max-w-lg rounded-2xl object-cover sm:mt-16 lg:mt-0 lg:max-w-none xl:row-span-2 xl:row-end-2 xl:mt-36"
        />
      </div>
    </div>
    <!-- Gradient at the bottom -->
    <div
      class="absolute inset-x-0 bottom-0 -z-10 h-24 bg-gradient-to-t from-white sm:h-32"
    />
  </section>
</template>

<script setup>
// No additional logic is required, so we don't need a script block for this component
</script>
